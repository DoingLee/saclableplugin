...
public class JdActivityMultiExt implements ActivityMultiExt {
    ...
    public void processStep1(ExtensionContext extensionContext, RequestData requestData, ResponseData responseData) {
        responseData.addData("Jd.processStep1", "京东活动复杂处理流程step1");
        extensionContext.getLogger().warn("Jd.processStep1 - 京东活动复杂处理流程step1");
        extensionContext.debug("Jd", "京东活动复杂处理流程step1");
    }

    @Override
    public void processStep2(ExtensionContext extensionContext, RequestData requestData, ResponseData responseData) {
        responseData.addData("Jd.processStep2", "京东活动复杂处理流程step2");
        extensionContext.getLogger().warn("Jd.processStep2 - 京东活动复杂处理流程step2");
        extensionContext.debug("Jd", "京东活动复杂处理流程step2");
    }
}
